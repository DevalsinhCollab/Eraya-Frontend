...
export const getPatients = createAsyncThunk('getPatients', async (data, { rejectWithValue }) => {
  try {
    const response = await axios.get(`${process.env.REACT_APP_BACKEND_API}/patient/getpatients`, {
      params: data,
      ...ApiHeaderWithToken(),
    });
...
export const deletePatient = createAsyncThunk('deletePatient', async (id, { rejectWithValue }) => {
  try {
